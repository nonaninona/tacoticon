깔끔하고 가독성 좋은 **README.md** 양식으로 작성해 드립니다.
그대로 복사해서 프로젝트 폴더에 `README.md` 파일을 만들고 붙여넣으시면 됩니다. 이미지 들어갈 자리는 `![설명](이미지경로)` 형태로 비워두었습니다.
...
## ✨ 주요 기능
  ...
  * **SSL 우회:** 사내 보안망이나 SSL 인증서 문제가 있는 환경에서도 작동하도록 강력한 우회 설정이 적용되어 있습니다.
  * **토큰 입력 방식:** 스크립트 실행 시 토큰을 입력받으므로 코드를 수정할 필요가 없습니다.
...
2.  필터 입력칸에 \*\*`api`\*\*라고 입력하거나 `Fetch/XHR`을 선택합니다. (복잡한 이미지 파일 등을 제외하고 보기 위함)
3.  이 상태에서 Mattermost 화면을 **새로고침(F5)** 하거나 다른 채널을 클릭합니다.
...
4.  **`Cookie`** 항목에 있는 **`MMAUTHTOKEN=xxxx...`** 값, 혹은 **`Authorization: Bearer xxxx...`** 값을 찾습니다.
...
2.  안내에 따라 **토큰**을 입력합니다. (보안을 위해 화면에 보이지 않을 수 있습니다.)
    `![토큰 입력 화면](./images/input_token.png)`
